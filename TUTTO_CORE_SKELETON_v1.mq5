//+------------------------------------------------------------------+
//| TUTTO Geometric Field OS - Core Skeleton v1.1                    |
//| Current-Timeframe Structure Priority Version                     |
//+------------------------------------------------------------------+
#property strict
...
input string             InpObjectPrefix   = "TUTTO_CORE_V11_";
...
void DrawCurrentSignals(const double &open[], const double &high[], const double &low[], const double &close[], const datetime &time[], const SwingState &state)
{
   ...
   bool local_low_rebound = (low[shift] <= state.last_low + bounce_zone &&
                             close[shift] > open[shift] &&
                             close[shift] > close[shift + 1]);

   bool local_high_reject = (high[shift] >= state.last_high - bounce_zone &&
                             close[shift] < open[shift] &&
                             close[shift] < close[shift + 1]);
   ...
   bool long_signal = (state.hl && local_low_rebound && ma_soft_long);
   bool short_signal = (state.lh && local_high_reject && ma_soft_short);
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "TUTTO Core Skeleton v1.1 - Current TF Priority");
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
{
   int minimum_bars = MathMax(InpMAPeriod + 5, InpSwingLookback + InpFractalDepth + 5);
   ...
   DrawCurrentSignals(open, high, low, close, time, state);

   double bias_score = BiasScore(close[1], g_ma_buffer[1], g_ma_buffer[10]);

   string state_text =
      "TUTTO Core Skeleton v1.1\n"
      "TF Structure Priority: CURRENT CHART ONLY\n"
      "Structure: " + HighLabel(state) + " / " + LowLabel(state) + "\n"
      "MA200 Bias Score: " + DoubleToString(bias_score, 2) + "\n"
      "MA Role: SOFT FILTER ONLY\n"
      "Long core: HL + current swing-low rebound\n"
      "Short core: LH + current swing-high rejection";
